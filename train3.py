...
class NSLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split_name == "train":
            # small Gaussian noise on real (non-padding) frames
            noise = np.random.normal(0, 0.01, seq[:actual_len].shape).astype(np.float32)
            seq[:actual_len] += noise

            # random scaling (simulates signer size/distance from camera)
            scale = np.random.uniform(0.95, 1.05)
            seq[:actual_len] *= scale

            # random temporal shift (shifts where the sign sits in the 100-frame window)
            max_shift = min(10, FIXED_LEN - actual_len)
            if max_shift > 0:
                shift = np.random.randint(0, max_shift + 1)
                shifted = np.zeros_like(seq)
                shifted[shift:shift + actual_len] = seq[:actual_len]
                seq = shifted
...
for epoch in range(EPOCHS):
    ...
    # ── Early stopping check (based on val loss, not acc) ────
    if val_loss < best_val_loss:
        ...
        if patience_counter >= patience:
            print(f"\n⏹ Early stopping at epoch {epoch+1} (no val loss improvement for {patience} epochs)")
            break
